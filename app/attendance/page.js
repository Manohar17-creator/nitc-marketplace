...
export default function AttendancePage() {
  ...
  const fetchSubjects = async () => {
    try {
      const response = await fetchWithAuth('/api/attendance/subjects')
  ...
  const fetchTodayAttendance = async () => {
    try {
      const response = await fetchWithAuth(`/api/attendance/get?date=${selectedDate}`)
      const data = await response.json()
  ...
  const handleDeleteSubject = async (subjectId) => {
    ...
    try {
      const response = await fetchWithAuth(`/api/attendance/subjects/${subjectId}`, {
        method: 'DELETE'
      })
