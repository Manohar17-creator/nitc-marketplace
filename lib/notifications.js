...
export async function sendTargetedNotification({ userIds, title, message, type = 'info', link = null }) {
  ...
  // 1. Save to DB (This part is working according to your logs)
  await db.collection('notifications').insertMany(objectIds.map(id => ({
    userId: id, title, message, type, link, read: false, createdAt: new Date()
  })));

  // 2. Fetch Tokens - Check both 'fcmToken' and 'token' fields
  const usersWithTokens = await db.collection('users').find({
    _id: { $in: objectIds },
    $or: [ { fcmToken: { $exists: true } }, { pushToken: { $exists: true } } ]
  }).project({ fcmToken: 1, pushToken: 1 }).toArray();

  const tokens = usersWithTokens.map(u => u.fcmToken || u.pushToken).filter(t => !!t);

  if (tokens.length === 0) {
    console.log("⚠️ No FCM tokens found for the selected users. Only DB records created.");
    return;
  }
  ...
  try {
    const response = await admin.messaging().sendEachForMulticast({
      ...
      data: { type, url: link || '/notifications' }
    });
    console.log(`✅ Firebase: ${response.successCount} sent successfully.`);
  } catch (err) {
