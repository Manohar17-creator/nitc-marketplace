...
export async function broadcastNotification({ title, message, type = 'info', link = null }) {
  ...
  // --- Step A: Send to Firebase (The actual Pop-up) ---
  const topic = 'all_users' // Make sure your frontend subscribes to this!
  ...
  const payload = {
    ...
    data: {
      type: type || 'info',
      url: link || '/announcements', // Handle click in service worker
  ...
  try {
    ...
    console.error('⚠️ Firebase Broadcast Failed:', error)
    // We continue execution so we still save to DB below
  }

  // --- Step B: Save to Database (The Bell Icon history) ---
