...
function canUseNotifications() {
  ...
  if (isIOSSafari()) {
    console.log('⚠️ iOS Safari detected - Push Notifications not supported')
    return false
  }
...
export async function requestNotificationPermission(userId) {
  ...
  if (!canUseNotifications()) {
    if (isIOSSafari()) {
      alert('📱 Notifications are not supported in iOS Safari. Please use Chrome, Firefox, or add this site to your Home Screen.')
    } else {
      alert('⚠️ Notifications are not available in your current browser mode. Try disabling Private Browsing.')
    }
    return null
  }
