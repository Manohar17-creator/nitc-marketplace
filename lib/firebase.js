...
/**
 * ⚡ NUCLEAR FIX: Uses Hardcoded Config to bypass Mobile Cache
 */
async function getTokenAndSave(userId) {
  try {
    console.log('📝 Registering service worker...')
    
    // 1. Unregister ALL existing workers to kill the "Ghost Worker"
    ...
    if (registration.installing || registration.waiting) {
        ...
        await new Promise(resolve => setTimeout(resolve, 2000)) // Give it time to skipWaiting
        activeReg = await navigator.serviceWorker.ready
    }

    console.log('✅ Service worker active. Initializing Clean Firebase App...')

    // 🔴 HARDCODED CONFIG (The Fix)
    // This bypasses the cached 'process.env' on your phone
    const cleanConfig = {
      apiKey: "AIzaSyA44DLUAwtcRLCNdf2GmiqxanKBYFWAgWo", // Your Working Key
    ...
    let cleanApp;
    try {
        // Try to retrieve if already exists, otherwise create
    ...
    while (!token && attempts < 5) {
      try {
        ...
        token = await getToken(messaging, {
          vapidKey: process.env.NEXT_PUBLIC_FIREBASE_VAPID_KEY, // VAPID key usually doesn't cache-break, but check this if it fails
