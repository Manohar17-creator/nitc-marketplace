...
async function getTokenAndSave(userId) {
  try {
    ...
    try {
      const messaging = getMessaging(cleanApp)
      await fetch(`https://iid.googleapis.com/iid/v1/${token}/rel/topics/all_users`, {
        method: 'POST',
        headers: {
          'Authorization': `key=${process.env.NEXT_PUBLIC_FIREBASE_SERVER_KEY}` // You need this
        }
      })
      console.log('✅ Subscribed to all_users topic')
    } catch (e) {
      console.error('⚠️ Topic subscription failed:', e)
    }
