...
async function getTokenAndSave(userId) {
  try {
    ...
    // 1. Register (We don't capture the variable here anymore)
    await navigator.serviceWorker.register('/firebase-messaging-sw.js', {
      scope: '/'
    })

    // 2. CRITICAL FIX: Wait for ready and CAPTURE the active registration
    // navigator.serviceWorker.ready resolves to the ACTUALLY ACTIVE registration
    ...
    const token = await getToken(messaging, {
      ...
      serviceWorkerRegistration: registration // ✅ Use the 'ready' registration
    })
    ...
    // Don't rethrow, just return null so the UI doesn't crash
    return null 
  }
}
